...
class Category {
    constructor(categoryDigit, label, description, lookups) {
        this._digit = categoryDigit; // string [0-9A-F]
        this._label = label || null; // string
        this._description = description || null; // string
...
class Reason {
    constructor(categoryDigit, reasonDigit, label, lookups) {
        this._digit = reasonDigit; // string [0-9A-F]
        this._label = label || null; // string
        this._lookups = lookups || [];
        this._hexString = this._getHexString(categoryDigit, reasonDigit); // string 0x[0-9A-F]
        this._hexValue = this._getHexValue(this._hexString); // int 0-255
    }
...
class Fission {
    ...
    parseString(hexString) {
        ...
        return {
            ...
            categoryLookups: category._lookups,
    ...
    _expandReasonLookups() {
        this.hexRange.forEach(categoryDigit => {
            ...
            this.hexRange.forEach(reasonDigit => {
                ...
                reason._lookups.forEach(lookup => {
    ...
    _expandCategoryLookups() {
        this.hexRange.forEach(categoryDigit => {
            const category = this.codes[categoryDigit];
            category._lookups.forEach(lookup => {
...
console.log(fission.codes.PERMISSION["ALLOWED"]._hexString); // 0x11 
console.log(fission.codes.CONTROL.GO._hexString);            // 0x11 (Alternate lookups)
